...
def test_successful_fetch(client, sample_av_response):
    with patch.object(client.session, "get", return_value=_mock_response(sample_av_response)):
...
def test_raises_rate_limit_error(client, rate_limit_response):
    with patch.object(client.session, "get", return_value=_mock_response(rate_limit_response)):
        with pytest.raises(RateLimitError):
            client.get_daily_stock_data("AAPL")


def test_raises_value_error_on_api_error(client, error_response):
    with patch.object(client.session, "get", return_value=_mock_response(error_response)):
...
def test_raises_value_error_on_missing_time_series(client):
    with patch.object(client.session, "get", return_value=_mock_response({"unexpected": "data"})):
        # Updated to match actual error message from api_client.py
